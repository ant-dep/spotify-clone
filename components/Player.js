...
function Player() {
  ...
  const handlePlayPause = () => {
    spotifyApi.getMyCurrentPlaybackState().then((data) => {
      ...
      {data.body?.is_playing ? setIsPlaying(false) : setIsPlaying(true)}
    });
  };
  ...
  useEffect(() => {
    if (currentTrackId && (volume > 0 && volume < 100)) {
  ...
  return (
    <>
      {currentTrackId && (
        ...
        <div className="h-24 bg-gradient-to-b from-zinc-900 to-black text-white grid grid-cols-3 text-sm md:text-base px-2 md:px-8">
          ...
          <div className="flex items-center space-x-3 md:space-x-4 justify-end p-5">
            ...
            <input
              ...
              min={0}
              max={100}
              className="w-14 md:w-36 volumeSlider"
            />
            <VolumeUpIcon
              className="w-5 h-5 cursor-pointer hover:scale-125 transition transform duration-100 ease-out"
