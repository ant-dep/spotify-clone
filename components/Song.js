    ...
    >
      <div className="flex items-center pl-3 space-x-4 py-1">
        ...
        <div>
          ...
          <p className="w-40 text-sm mt-1">{track.track.artists[0].name}</p>
        </div>
      </div>

      <div className="flex items-center justify-between ml-auto md:ml-0 pr-10">
        <p className="hidden md:inline w-40 lg:w-96 truncate text-sm">
          {track.track.album.name}
        </p>
        <p className='text-sm'>{millisToMinutesAndSeconds(track.track.duration_ms)}</p>
      </div>
    </div>
  );
}
